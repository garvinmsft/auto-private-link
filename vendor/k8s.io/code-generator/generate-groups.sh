...
(
  ...
  cd "$(dirname "${0}")"
  go install -a ./cmd/{defaulter-gen,client-gen,lister-gen,informer-gen,deepcopy-gen}
)
...
if [ "${GENS}" = "all" ] || grep -qw "deepcopy" <<<"${GENS}"; then
  ...
  "${gobin}/deepcopy-gen"  -v 5 --input-dirs "$(codegen::join , "${FQ_APIS[@]}")" -O zz_generated.deepcopy --bounding-dirs "${APIS_PKG}" "$@"
fi
